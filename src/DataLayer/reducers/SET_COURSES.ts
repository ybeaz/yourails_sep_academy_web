...
import { CourseType, ModuleType } from 'yourails_common'

export const SET_COURSES: ReducerType = (store: RootStoreType, data: any): RootStoreType => {
  const { isLoaded } = store
  const { mediaLoaded } = isLoaded

  /* Making mediaLoaded false for each moduleID by default */
  const mediaLoadedNext = data.reduce((accum: Record<string, boolean>, course: CourseType) => {
